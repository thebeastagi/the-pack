...
## 12. Monetisation phase 1 — den-fire credits (v0.7.0, 2026-07-21)

Spec: `/shared/deliverables/pack-monetisation-plan-2026-07-21/MONETISATION-PLAN.md` (Robin-approved). Phase 1 = credits + buy-credits only (Den Pro, voice credits, x402 = later phases).

**Credits (migration 0008)** — `credit_balances` (1 credit = $0.01 list; prepaid, non-transferable, no cash-out), `credit_ledger` (every mutation, `balance_after` read post-mutation inside the same batch), `payment_orders` (SKU, amount, status; `UNIQUE(provider, provider_ref)`). `src/credits.js`: `atomicDebit` = guarded `UPDATE … WHERE balance >= ?` (negative balances and raced double-spends are impossible at the SQL level; a lost race changes 0 rows → insufficient, phantom ledger row compensated), `grantCredits`, `settleBurn`. All mutations + their ledger rows execute in ONE D1 batch (transactional).

**Pricing** — packs: Spark $5/500, Ember $10/1,100, Fire $20/2,500, Inferno $50/7,000. Burn floors: search 5 cr, image 4 cr. The actual charge is USD-tick denominated: `max(floor, ceil(real_ticks × PRICE_MULTIPLIER(2.0) × 100 / 1e10))` — pre-call the flat floor is debited, post-call `settleBurn` tops up when the real billed cost runs hotter (self-healing against xAI repricing; the $0.002→$0.020 image lesson institutionalized).

**Two-layer caps (`src/caps.js`)** — the existing `PACK_*` caps are now HARD infrastructure ceilings (absolute for everyone, paying or not; `daily_usd_cap`, `den_hard_cap`, `global_hard_cap`, `usage_read_failed` never burn). New `PACK_FREE_*` vars define the free pool (search 20/den/day, image 3/den/day, $2/day global free-spend fuse): a free-pool denial (`den_cap`/`global_cap`) — and ONLY those — falls through to a credit burn via `brainAllowedOrBurn`. RAG/voice have no phase-1 burn path (no burn floor ⇒ denial never debits).

**Payments (`src/payments.js`)** — AllScale hosted checkout on the existing Beast store with `pack:{userId}:{uuid}` order namespacing. `POST /api/payments/allscale/create-intent` (session/agent-key auth — tighter than the dashboard's public version by design; per-IP 6/min + per-user 10/day + global 120/min; SKU-enum amounts, server-generated order refs). Settle paths, one shared exactly-once core (`settleOrder` — grant + ledger guarded by `EXISTS(order still 'created')` inside the batch transaction; replay/race = zero side effects → 409):
1. **Webhook** (`POST /api/payments/allscale/webhook`): HMAC-verified per AllScale's documented canonical (`allscale:webhook:v1` prefix, raw-body SHA-256, ±300 s, timing-safe compare, payload `webhook_id` must match the signed header). 401 unsigned/forged/stale, 404 unknown order (safe no-op), 422 amount/order_ref mismatch (never grants). Registered in `worker.js` BEFORE the Access gate + `ACCESS_BYPASS_PATHS` + CF Access bypass app `682b2f2c` (edge/worker lockstep).
2. **Reconcile** (`POST /api/payments/orders/{id}/reconcile`, owner-only): signed `GET /v1/checkout_intents/{id}` — status 20 (CONFIRMED) settles. This is the **primary path while the shared store's single webhook URL points at the dashboard worker**; `/pay/thanks` polls it (bounded). Long-term webhook topology (dedicated store / multi-endpoint / forwarder) is an open Robin decision.

**Pages** — `/pay` (den-fire storefront, pack cards, live balance + ledger + orders, honest terms: non-refundable prepaid units, 18+; card copy deliberately conservative — no card settlement has been proven on Beast rails) and `/pay/thanks` (settle watcher). The layout shows a 🔥 credits pill for signed-in citizens. `GET /api/credits` (auth) = balance + last 20 ledger rows + last 10 orders. Every settle records a signed `payment_settled` memory episode (provenance on money movement).

**Secrets** — `ALLSCALE_API_KEY` / `ALLSCALE_API_SECRET` via `wrangler secret put` only. Until they are set the worker is deliberately raise-safe: checkout/reconcile → honest 503, webhook → fail-closed 503 (never grants), `/pay` shows a "being wired up" state with disabled buttons.
