...
export const SQL = {
  ...
  denArtPut:
    "INSERT INTO den_art (den_id, mime, bytes, created_at) VALUES (?, ?, ?, ?) ON CONFLICT(den_id) DO UPDATE SET mime = excluded.mime, bytes = excluded.bytes, created_at = excluded.created_at",
  denArtGet: "SELECT mime, bytes FROM den_art WHERE den_id = ?",
...
// ── den artwork (Runway-generated bytes; art_url marks presence for the UI) ──
export async function putDenArt(db, { denId, mime, bytes, artUrl }) {
  await db.prepare(SQL.denArtPut).bind(denId, mime, bytes, nowIso()).run();
  await db.prepare(SQL.denSetArtUrl).bind(artUrl, denId).run();
}

export async function getDenArt(db, denId) {
  return db.prepare(SQL.denArtGet).bind(denId).first();
}
